...
Public Class reg
    ...
    Private Sub btnst_Click(sender As Object, e As EventArgs) Handles btnst.Click
        ...
        ElseIf checkphone(txtphn.Text.Trim) = False Then
            MsgBox("Please enter a valid phone Number")
            txtphn.Clear()
        ElseIf IsEmailValid(txtml.Text.Trim) = False Then
    ...
    Private Function IsEmailValid(p1 As String) As Boolean

        Try
            Dim maddr As MailAddress
            maddr = New MailAddress(p1)
        Catch ex As Exception
            Return False
        End Try
        Return True
    ...
    Private Function checkphone(p1 As String) As Boolean
        Dim phoneNumber As New Regex("\d{10}")
        If phoneNumber.IsMatch(p1) Then
            Return True
        Else
            Return False
        End If
